...
error data leaves the server. The published web-app image reports nowhere: it
is built without `NEXT_PUBLIC_SENTRY_DSN`, and without that value the
reporting SDK is not in the bundle. To have it report, build the client image
yourself ([Where the images come from](#where-the-images-come-from)) with the
build arg `NEXT_PUBLIC_SENTRY_DSN` set. `docker-compose.selfhost.build.yml`
passes it from the variable of the same name, so put the line in `.env` and
every later rebuild keeps it:

```bash
echo 'NEXT_PUBLIC_SENTRY_DSN=https://key@glitchtip.example.com/2' >> .env
docker compose -f docker-compose.selfhost.yml -f docker-compose.selfhost.build.yml up -d --build
```

The value is inlined at build time, so setting it on a running container does
nothing. What a report carries is
...
| Desktop app | `app://-` |
| Firefox extension | `moz-extension://<uuid>` — by shape, see below |
...
**The Firefox extension is the one that cannot be listed.** Firefox gives every
install its own `moz-extension://<uuid>` address, so no trust list can hold it
— a value that works on your machine is wrong on everybody else's. Your server
accepts that shape of address instead, and only for requests that carry no
session cookie: the extension signs in with a token, and a request from such an
address never gets a credentialed CORS answer. That is what stops any other
extension in your browser from using your signed-in session, on the live
connection in particular, where cookies cannot be switched off.

`TRUST_EXTENSION_ORIGINS` is that switch. Unset, it follows
`TRUST_STORE_APPS`, so the compose file accepts the Firefox extension out of
the box. Set `TRUST_EXTENSION_ORIGINS=false` to accept the Chrome extension
and refuse the Firefox one, or `true` on a server that keeps
`TRUST_STORE_APPS=false` and lists its origins by hand.
...
`TRUST_STORE_APPS=false`, the popup says so and names the setting — for
Firefox, `TRUST_EXTENSION_ORIGINS`, because its address cannot go in
`TRUSTED_ORIGINS`. The extension keeps its unsent changes until the server
accepts it again.

**Firefox works the same way, with two differences.** Signing in on your web
app never signs the extension in, on any server: Firefox has no way for a page
to message an extension. Use the popup's password form, or **Sign in with the
web app**. And the live connection needs `https://` — Firefox blocks an
extension from opening an insecure `ws://` connection, even to your own
machine, so an `http://` server gives you a working extension with no live
...
| [`.github/workflows/release.yml`](../.github/workflows/release.yml) | On a `v*` tag, builds both images for `amd64` and `arm64`, pushes the exact tag, starts the stack from an anonymous pull on both, then moves `X.Y` and `latest` |
