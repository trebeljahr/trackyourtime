...
import { invoiceT, type InvoiceTranslator } from "./i18n.js";
import { billedPeriodDates } from "./format.js";
import {
  bankLines,
  breakdownTotalRows,
  exemptionReasons,
...
/**
 * The issuer's logo as the renderer reads it: raw image bytes. The server's
 * `StoredLogo` (with mime, size and a sha256) is a superset, so a server
 * invoice satisfies this without conversion; the public generator page hands
 * over the uploaded bytes directly.
 */
export type RenderableInvoiceLogo = { data: Uint8Array };

/**
 * What `renderInvoicePdf` draws from: the wire invoice with the issuer logo's
 * bytes kept. A plain wire `Invoice` satisfies it too (the key is optional),
 * so an invoice without a logo renders exactly as before.
 */
export type RenderableInvoice = Omit<Invoice, "issuer"> & {
  issuer?: (InvoiceIssuer & { logo?: RenderableInvoiceLogo | null }) | null;
};

/** pdfkit's `image()` accepts these; a `Uint8Array` is one at runtime. */
type PdfImageSource = Parameters<PDFKit.PDFDocument["image"]>[0];
...
/** Join the emitted chunks without `Buffer`, so the package stays node-free. */
...
function drawLogo(sheet: Sheet): { titleWidth: number; contentTop: number } {
  ...
  // pdfkit's `image()` types accept Buffer | ArrayBuffer | string; the raw
  // bytes are a Uint8Array, which it reads at runtime just the same.
  const bytes = logo.data as unknown as PdfImageSource;
  sheet.doc.image(bytes, sheet.left + sheet.width - LOGO_BOX.width, sheet.y, {
 ...
 *
 * pdfkit is a stream (a Node stream on the server, a shim in the browser
 * build), so this is where callback-land is bridged back into a promise.
 * `doc.end()` has to be called before the await, otherwise the "end" event
 * never fires and the promise hangs. The bytes come back as a `Uint8Array` so
 * this module never touches `Buffer`; the server wraps the result in
 * `Buffer.from` at its own entry point.
 */
