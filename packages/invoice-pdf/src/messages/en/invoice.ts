/**
 * English `invoice` — the SOURCE catalog for the invoice PDF (invoice-pdf.ts): headings, column labels, totals, tax and due-date lines.
 *
 * Add keys here, then the same keys in ../de/invoice.ts (`tsc` enforces it). ICU
 * syntax as in the web client. The text is rendered in a locale decided by the
 * DOCUMENT (an invoice's snapshotted `locale`), never by the request that
 * happens to trigger it. The server's `serverT(locale, "invoice")` re-exports
 * this catalog, so `tests/i18n-catalog.test.ts` parity still covers it.
 *
